 ...
 *
 * Description: If the file is not an ELF file - exit code -1.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
...
/**
 * print_class - A function thay prints the class of an ELF header.
 * @e_ident: A pointer to an array containing the ELF class.
 */
void print_class(unsigned char *e_ident)
{
	printf("  The Class of ELF:                             ");
...
void print_data(unsigned char *e_ident)
{
	...
	case ELFDATANONE:
		...
		break;
	case ELFDATA2LSB:
		printf("2's Complement, The little endian\n");
		break;
	case ELFDATA2MSB:
		printf("2's Complement, The big endian\n");
		break;
	default:
		printf("<unknown ELF Class: %x>\n", e_ident[EI_CLASS]);
	}
